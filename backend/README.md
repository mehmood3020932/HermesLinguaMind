...
The nginx container now serves the Hermes LinguaMind website at `http://localhost/` and reverse-proxies the live gateway on the same origin. The website's live tutor panel calls `POST /v1/chat`, reads `GET /health`, and discovers services from `GET /v1/services`.

The previous standalone frontend demo is retained at `backend/frontend/legacy-demo/` for reference.
